...
# 1 "../mylib/include\\config.h" 1
# 24 "../mylib/include\\config.h"
...
# 1 "../mylib/include\\display3.h" 1
# 20 "../mylib/include\\display3.h"
    void numarator(void);
    void test_segments(unsigned char);
...
unsigned char character_table(unsigned char symbol){
    typedef const unsigned char c_t;
    ...
    switch (symbol){
        ...
        case 'a': return(unsigned char) ~(d|h);
        ...
        case 0 : return(unsigned char) o;
        case 'o': return(unsigned char) o;
        case '*': return (unsigned char) a|f|g|h;
        default: return (unsigned char) ~(c|d);
    }
}

void send_digit(unsigned char data){
    unsigned char a;
    POZ com_mask=SER|SRCLK|SRCLR|RCLK ;
    LATB&=~com_mask;
...
void display3(const char* data3){
    ...
    while( bit & dig_mask ) {
        ...
        send_digit( character_table(*(data3++)) );
        LATB |= dig_mask;
        LATB=(LATB & ~(SER|SRCLK|SRCLR|RCLK)) | RCLK; LATB=(LATB & ~(SER|SRCLK|SRCLR|RCLK));;
    ...
    send_digit('o');_delay((unsigned long)((20)*(8000000/4000000.0))); LATB|=dig_mask;

}


void __attribute__((picinterrupt(("high_priority")))) my_ISR(){
...
void test_segments(unsigned char poz){
    ...
    LATB=(unsigned char)~(DIG1 | DIG2 | DIG3);
    while (1){
        send_digit(test_bit); LATB=(LATB & ~(SER|SRCLK|SRCLR|RCLK)) | RCLK; LATB=(LATB & ~(SER|SRCLK|SRCLR|RCLK));; LATB &=~OE;
